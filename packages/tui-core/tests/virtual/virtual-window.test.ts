...
describe("VirtualListMachine", () => {
  it("clamps scrolling within content bounds", () => {
    const m = new VirtualListMachine({
      itemCount: 100,
      itemHeight: 1,
      viewportHeight: 10,
    });
...
describe("VirtualListMachine resize", () => {
  it("reclamps scroll when the item count changes", () => {
    const m = new VirtualListMachine({
      itemCount: 100,
      itemHeight: 1,
      viewportHeight: 10,
    ...
    m.setItemCount(20);
    expect(m.maxScroll).toBe(10);
    expect(m.scrollTop).toBe(10);

    m.setItemCount(200);
    expect(m.maxScroll).toBe(190);
    expect(m.scrollTop).toBe(10);
  });

  it("reclamps scroll when the viewport height changes", () => {
    const m = new VirtualListMachine({
      itemCount: 100,
      itemHeight: 1,
      viewportHeight: 10,
    ...
    expect(m.maxScroll).toBe(80);
    expect(m.scrollTop).toBe(80);

    m.setViewportHeight(200);
  ...
  it("rejects negative or fractional dimensions", () => {
    ...
    expect(() => m.setItemCount(-1)).toThrow();
    expect(() => m.setViewportHeight(1.5)).toThrow();
  });
});
