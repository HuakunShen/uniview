...
flowchart TB
    ...
    subgraph Plugin["PLUGIN (Web Worker/Node.js)"]
        Runtime["@uniview/runtime"]
        PluginRuntime["PluginRuntime<br/>(RPC Handler)"]
        ReactRenderer["@uniview/react-renderer<br/>Custom Reconciler → UINode"]
        PluginCode["PLUGIN CODE (React)<br/>function App()"]
...
sequenceDiagram
    ...
    Note over Plugin: Plugin renders React app
...
sequenceDiagram
    ...
    Note over Plugin: Plugin executes handler<br/>React state changes<br/>Reconciler produces new tree
    Plugin->>Host: updateTree(UINode)
...
sequenceDiagram
    ...
    Note over Plugin: Plugin receives new props<br/>React re-renders with props
    Plugin->>Host: updateTree(UINode)
```
...
Unlike the traditional approach where each plugin runs its own WebSocket server, Uniview uses a single Elysia-based bridge server. This simplifies deployment and networking by multiplexing all plugin connections through a single port.
